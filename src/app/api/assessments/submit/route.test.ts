...
describe("POST /api/assessments/submit", () => {
  ...
  it("rejects an unknown template", async () => {
    const response = await POST(
      makeRequest({ templateSlug: "nao-existe", answers: PHQ9_ZEROS })
    );
  ...
  it("rejects a wrong number of answers", async () => {
    const response = await POST(
      makeRequest({ templateSlug: "phq9", answers: [0, 1] })
    );
    expect(response.status).toBe(400);
  });
  ...
  it("rejects an out-of-range answer", async () => {
    ...
    answers[0] = 99;
    const response = await POST(
      makeRequest({ templateSlug: "phq9", answers })
    );
    expect(response.status).toBe(400);
  });
  ...
  it("returns 503 when Supabase isn't configured yet", async () => {
    const response = await POST(
      makeRequest({ templateSlug: "phq9", answers: PHQ9_ZEROS })
    );
